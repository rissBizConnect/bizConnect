...
//		logger.info("after encode : " + member.getUserPwd() + ", length : " + member.getUserPwd().length());		
//		
...
//	}
	...
	// 회원 가입 요청 처리용 메소드 (사진파일 첨부 기능이 있는 경우 처리 방식) => 첨부된 파일을 별도로 전송받도록 처리함
...
//		member.setUserPwd(encodePwd);
		...
		if (!mfile.isEmpty()) {
			...
			// 여러 회원이 업로드한 사진파일명이 중복 될 경우를 대비해서 저장파일명 이름바꾸기함
			// 바꿀 파일 이름은 개발자가 정함
			// userId 가 기본키이므로 중복 안됨 => userId_filename 저장형태로 정해봄
			...
			// 파일 업로드 정상 처리되었다면
			member.setPhotoFileName(renameFileName); // db 저장시에는 원래 이름으로 기록함
		} // 첨부파일이 있을 때
	...
	public String memberDetailMethod(@RequestParam("userId") String userId, Model model) {
		...
		if (member != null) { // 전송온 아이디로 회원 조회 성공시
			// 첨부된 사진파일 있다면, 원래 파일명으로 변경해서 뷰로 전달함
