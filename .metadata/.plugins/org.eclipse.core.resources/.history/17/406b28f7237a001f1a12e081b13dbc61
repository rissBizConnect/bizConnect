...
window.onload = function(){
	...
	photofile.addEventListener("change", function(event){
		...
		//클라이언트 컴퓨터 있는 파일은 src 속성으로 적용할 수 없음
		...
		reader.onload = (e) => { //e : event 객체
			myphoto.setAttribute("src", e.target.result); //e.target : 이벤트 발생 대상(읽어들인 파일)
...
function validate(){
	//수정하기 보내기 전에 입력값들 유효한 값인지 확인하는 작업
...
<table id="outer" align="center" width="700" cellspacing="5" cellpadding="0">	
	...
	<tr><th>나 이</th><td><input type="number" name="age" min="19" value="${requestScope.member.age }" ></td></tr>
	<tr><th>전화번호</th><td><input type="tel" name="phone" value="${requestScope.member.phone }" ></td></tr>
	<tr><th>이메일</th><td><input type="email" name="email" value="${requestScope.member.email }" ></td></tr>
	<tr><th colspan="2">
		<input type="submit" value="수정하기"> &nbsp; 
		<%-- <a href="mdelete.do?userid=${requestScope.member.userId}">탈퇴하기</a> --%>
		<c:url var="mdel" value="medlete.do">
