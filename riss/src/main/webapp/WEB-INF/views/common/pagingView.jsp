...
<div style="text-align: center;">
	...
	<c:if test="${ currentPage gt 1 }">
		<a href="/bizconnect/${ urlMapping }?page=1&fileter=${ fileter }&begin=${ begin }&end=${ end }&smember=${smember}">[첫페이지로]</a> &nbsp;
	</c:if>
	
	<%-- 이전 페이지그룹으로 이동 --%>
	<%-- 이전 그룹이 있다면 --%>
	<c:if test="${ (currentPage - 10) lt startPage and (currentPage - 10) gt 1 }">
		<a href="/bizconnect/${ urlMapping }?page=${ startPage - 10 }&fileter=${ fileter }&begin=${ begin }&end=${ end }&smember=${smember}">[이전그룹]</a> &nbsp;
	</c:if>
	<%-- 이전 그룹이 없다면 --%>
	<c:if test="${ !((currentPage - 10) lt startPage and (currentPage - 10) gt 1) }">
		[이전그룹] &nbsp;
	...
	<c:forEach begin="${ startPage }" end="${ endPage }" step="1" var="p">
		...
		<c:if test="${ p ne currentPage }">
			<a href="/bizconnect/${ urlMapping }?page=${ p }&fileter=${ fileter }&begin=${ begin }&end=${ end }&smember=${smember}">${ p }</a>
			<%-- 예: 페이지 7 클릭시 href="/bizconnect/nlist.do?page=7" 요청 처리됨 --%>
		</c:if>
	</c:forEach>
	...
	<%-- 다음 그룹이 있다면 --%>
	<c:if test="${ (currentPage + 10) gt endPage and (currentPage + 10) lt maxPage }">
		<a href="/bizconnect/${ urlMapping }?page=${ startPage + 10 }&fileter=${ fileter }&begin=${ begin }&end=${ end }&smember=${smember}">[다음그룹]</a> &nbsp;
	</c:if>
	<%-- 다음 그룹이 없다면 --%>
	<c:if test="${ !((currentPage + 10) gt endPage and (currentPage + 10) lt maxPage) }">
		[다음그룹] &nbsp;
	...
	<c:if test="${ currentPage lt maxPage }">
		<a href="/bizconnect/${ urlMapping }?page=${ maxPage }&fileter=${ fileter }&begin=${ begin }&end=${ end }&smember=${smember}">[맨끝페이지로]</a> &nbsp;
	</c:if>
</div>
</c:if>
</body>
