	...
	<div class="front">
		...
		<div class="contentPos">
			...
			<form action="retList.do" method="get">
				GID: <input type="text" name="gid" value="${param.gid}" /> 회사 코드: <input
					type="text" name="comCode" value="${param.comCode}" /> 퇴직자 사번: <input
					type="text" name="retNo" value="${param.retNo}" /> 입사일: <input
					type="date" name="userEntryDate" value="${param.userEntryDate}" />
				...
				<tbody>
					<c:forEach var="retire" items="${list}">
						<tr id="retire-${retire.retNo}">
							...
							<td><a href="retDelete.do?retNo=${retire.retNo}&comCode=${retire.comCode}"
       							onclick="return confirm('정말로 삭제하시겠습니까?');">삭제</a></td>
							<td>${retire.status == 'Y' ? '사원' : '퇴직자'}</td>
							<td>
								<form
									action="${pageContext.request.contextPath}/updateRetireToMember.do"
									method="post">
									<input type="hidden" name="retNo" value="${retire.retNo}" /> <input
										type="hidden" name="gid" value="${retire.gid}" /> <input
										type="hidden" name="comCode" value="${retire.comCode}" /> <input
										type="submit" value="사원으로 변환" />
								</form>
							</td>
						</tr>
					</c:forEach>
